module.exports = async function (context, req) {
  ...
  if (req.query.name) { matchOperators.$match.name = { $regex: `^${req.query.name}.*`, $options: 'i' }; }
  if (req.query.condition) { matchOperators.$match.condition = { $eq: req.query.condition }; }
  if (req.query.sex) { matchOperators.$match.sex = { $eq: req.query.sex }; }
  ...
  if (req.query.sort == 'asc') { aggregate.sort({ name: 1 }); } else { aggregate.sort({ name: -1 }); }
  ...
  if (req.query.limit) { aggregate.limit(parseInt(req.query.limit)); }
