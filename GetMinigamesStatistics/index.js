module.exports = async function (context, req) {
  ...
  if (req.query.minigameName) { minigameName = req.query.minigameName; }

  if (req.query.devices) devices = [req.query.devices];
  else devices = ['Pitaco'];

  const minigameFlowOperator = minigameName == 'WaterGame' ? { $min: '$flow' } : { $max: '$flow' };
  ...
  try {
    ...
    context.res = {
      status: 200,
      body: responseUtils.createResponse(true, true, infoMessages.SUCCESSFULLY_REQUEST, minigamesStatistics),
    };
  } catch (err) {
